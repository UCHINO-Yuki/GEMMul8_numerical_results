...
%% files
fileList_s = dir("oz2_results_s_accuracy_*");
filename_s = strings(length(fileList_s),1);
for i=1:length(fileList_s)
    filename_s(i) = string(fileList_s(i).name);
end
...
if any(contains(filename_s,'GH200')) && any(contains(filename_d,'GH200'))
    fig = figure;
    ...
    filename = filename_d(contains(filename_s,'GH200'));
    ...
    filename = filename_s(contains(filename_s,'GH200'));
    opts = detectImportOptions(filename);
    opts.SelectedVariableNames = 3:length(opts.SelectedVariableNames);
    err = readmatrix(filename,opts);
    err = err(2:end,:);
    ...
    if saveflag
        ...
        figname = match{1}{1};
        figname = replace(figname, "_s_", "_ds_");
...
%% float
for fn = 1:length(filename_s)
    filename = filename_s(fn);
    opts = detectImportOptions(filename);
    opts.SelectedVariableNames = 3:length(opts.SelectedVariableNames);
    err = readmatrix(filename,opts);
    err = err(2:end,:);
    ...
    elseif contains(env,"B200")
        title(lgd, "B200", 'FontSize',FontSize+1);
    elseif contains(env,"MI300X")
        title(lgd, "MI300X", 'FontSize',FontSize+1);
    end
    xlabel(t,"Number of moduli",'FontSize',FontSize+1);
    ylabel(t,"max relative error",'FontSize',FontSize+1);
    t.TileSpacing = "tight";
